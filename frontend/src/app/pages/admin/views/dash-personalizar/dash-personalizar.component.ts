    ...
    <div class="space-y-6 animate-fade">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-white/10">
        <div>
          ...
          <p class="text-xs text-gray-400 mt-1 font-sans">Configura textos promocionales, información de contacto directa y alertas globales del portal.</p>
        </div>
        <button (click)="guardarConfiguracion()" 
                class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white hover:bg-gray-200 text-black border border-white font-extrabold text-xs transition-all shadow-lg scale-100 hover:scale-105 cursor-pointer">
          <span>✓ Guardar y Publicar Cambios</span>
        </button>
      </div>

      <!-- Alerta de éxito -->
      <div *ngIf="guardadoExitoso()" class="p-4 rounded-xl bg-white/5 border border-white/20 text-white text-xs flex items-center justify-between animate-fade font-mono">
        <div class="flex items-center gap-2 font-bold">
          <span class="w-1.5 h-1.5 rounded-full bg-white animate-pulse"></span>
          <span>¡Cambios publicados en vivo exitosamente! La configuración del sitio se ha actualizado.</span>
        </div>
        <button (click)="guardadoExitoso.set(false)" class="text-gray-400 hover:text-white font-bold p-1">✕</button>
      </div>

      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        
        <!-- Formulario de Configuración (Col 1) -->
        <div class="relative bg-[#030303] border border-white/10 rounded-xl p-6 shadow-xl space-y-5 text-xs font-sans overflow-hidden">
          ...
          <div class="flex items-center gap-3 py-1 font-sans">
            ...
            <label for="chkBanner" class="text-white font-bold cursor-pointer select-none">Activar Banner Superior de Anuncios</label>
          </div>
        ...
        <div class="space-y-6">
          <div class="relative bg-[#030303] border border-white/10 rounded-xl p-6 shadow-xl overflow-hidden">
            ...
            <div class="flex items-center justify-between mb-4 pb-2 border-b border-white/10 font-sans">
              ...
              <span class="text-[10px] text-gray-500 font-mono">Vista previa en tiempo real</span>
            </div>

            <!-- Muestra del Banner de Alerta -->
            <div *ngIf="formConfig.mostrarBannerAlerta" class="p-3 rounded-lg bg-white/5 border border-white/20 text-white text-xs font-mono mb-4 flex items-center justify-between">
              <span>{{ formConfig.textoBannerAlerta }}</span>
              <span class="text-[9px] font-bold bg-white text-black px-1.5 py-0.5 rounded">ALERTA</span>
            </div>

            <!-- Muestra del Hero -->
            <div class="p-6 rounded-xl bg-[#000000] border border-white/5 space-y-3 text-center">
              <span class="inline-block px-3 py-1 rounded-full border border-white/10 bg-[#161616] text-white text-[9px] font-mono tracking-widest uppercase">
                {{ formConfig.heroTagline }}
              </span>
              <h4 class="font-serif text-2xl font-bold text-white">Construimos la Residencia <br/><span class="text-gray-400">de tus Sueños</span></h4>
              <p class="text-xs text-gray-400 max-w-sm mx-auto font-sans">{{ formConfig.heroSubtagline }}</p>
            </div>
          </div>

          <!-- Muestra de Contacto -->
          <div class="relative bg-[#030303] border border-white/10 rounded-xl p-6 shadow-xl space-y-3 overflow-hidden">
            <!-- Trazos esquineros de plano -->
            <div class="absolute -top-1 -left-1 w-5 h-5 border-t border-l border-white/20 pointer-events-none"></div>
            <div class="absolute -bottom-1 -right-1 w-5 h-5 border-b border-r border-white/20 pointer-events-none"></div>
            ...
            <div class="space-y-2 text-xs font-sans">
              <div class="flex justify-between p-2.5 rounded-lg bg-[#0C0C0C]">
                <span class="text-gray-400">Teléfono:</span>
                <span class="text-white font-bold font-mono">{{ formConfig.telefonoContacto }}</span>
              </div>
              <div class="flex justify-between p-2.5 rounded-lg bg-[#0C0C0C]">
                <span class="text-gray-400">Correo Electrónico:</span>
                <span class="text-white font-bold font-mono">{{ formConfig.emailSoporte }}</span>
              </div>
              <div class="flex justify-between p-2.5 rounded-lg bg-[#0C0C0C]">
                <span class="text-gray-400">Instagram:</span>
                <span class="text-white font-bold font-mono">{{ formConfig.instagramHandle }}</span>
              </div>
            </div>
          </div>
        </div>
