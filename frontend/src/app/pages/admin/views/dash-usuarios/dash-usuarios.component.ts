...
@Component({
  ...
  template: `
    <div class="space-y-6 animate-fade">
      ...
      <!-- Barra de Búsqueda y Filtros con trazos arquitectónicos -->
      <div class="relative bg-[#030303] border border-white/10 rounded-xl p-4 shadow-lg flex flex-col sm:flex-row items-center justify-between gap-4 overflow-hidden">
        ...
        <div class="relative w-full sm:w-80">
          ...
          <input type="text" [(ngModel)]="terminoBusqueda" (ngModelChange)="actualizarFiltros()" placeholder="Buscar por nombre o correo..." 
                 class="w-full pl-10 pr-4 py-2.5 rounded-lg bg-[#090909] border border-white/10 text-white text-xs placeholder-gray-500 focus:border-white focus:outline-none">
        </div>

        <!-- Filtros de Rol -->
        <div class="flex items-center gap-2 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0 scrollbar-thin font-sans">
          <button (click)="setFiltroRol('todos')"
                  [ngClass]="filtroRol() === 'todos' ? 'bg-white text-black font-extrabold shadow-sm border border-white' : 'bg-[#0D0D0D] text-gray-400 hover:text-white'"
          ...
          <button (click)="setFiltroRol('propietario')"
                  [ngClass]="filtroRol() === 'propietario' ? 'bg-white text-black font-extrabold shadow-sm border border-white' : 'bg-[#0D0D0D] text-gray-400 hover:text-white'"
          ...
          <button (click)="setFiltroRol('arquitecto')"
                  [ngClass]="filtroRol() === 'arquitecto' ? 'bg-white text-black font-extrabold shadow-sm border border-white' : 'bg-[#0D0D0D] text-gray-400 hover:text-white'"
                  class="px-3.5 py-2 rounded-lg text-[11px] transition-all cursor-pointer whitespace-nowrap inline-flex items-center gap-1.5">
          ...
          <button (click)="setFiltroRol('inversionista')"
                  [ngClass]="filtroRol() === 'inversionista' ? 'bg-white text-black font-extrabold shadow-sm border border-white' : 'bg-[#0D0D0D] text-gray-400 hover:text-white'"
                  class="px-3.5 py-2 rounded-lg text-[11px] transition-all cursor-pointer whitespace-nowrap inline-flex items-center gap-1.5">
        ...
        <div class="overflow-x-auto scrollbar-thin">
          <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-white/5 text-xs font-sans">
              <tr *ngFor="let u of usuariosFiltrados()" class="hover:bg-[#0D0D0D] transition-colors group">
                
                <!-- Col 1: Nombre y Email -->
                <td class="py-3.5 px-5">
                  <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-full bg-white/5 border border-white/10 flex items-center justify-center font-serif font-bold text-white shrink-0">
                      {{ u.nombre.charAt(0) }}
                    </div>
                    <div>
                ...
                <td class="py-3.5 px-4 font-mono">
                  <span [ngClass]="u.rol === 'admin' ? 'bg-white text-black border-white' : 'bg-white/10 text-white border-white/20'"
                        class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded border text-[9px] font-bold uppercase tracking-wider">
                    {{ u.rol }}
                  </span>
                </td>
                ...
                <td class="py-3.5 px-4 font-mono font-bold text-gray-200">
                  <span class="bg-[#1C1C1C] px-2.5 py-1 rounded border border-white/5">{{ u.proyectosGuardados }} CADs</span>
                </td>
                ...
                <td class="py-3.5 px-4">
                  <button (click)="adminService.toggleEstadoUsuario(u.id)"
                          [ngClass]="{
                            'bg-white/10 text-white border-white/25': u.estado === 'activo',
                            'bg-amber-950/40 text-amber-300 border-amber-800/40': u.estado === 'pendiente',
                            'bg-red-950/40 text-red-300 border-red-800/40': u.estado === 'suspendido'
                          }"
                          class="px-3 py-1 rounded-xl border text-[10px] font-mono font-bold inline-flex items-center gap-1.5 cursor-pointer transition-transform hover:scale-105">
                    <span class="w-1.5 h-1.5 rounded-full" [ngClass]="u.estado === 'activo' ? 'bg-white' : 'bg-red-400'"></span>
                ...
                <td class="py-3.5 px-5 text-right">
                  <button (click)="adminService.eliminarUsuario(u.id)" title="Retirar Usuario de Sysmicon" 
                          class="px-3 py-2 rounded-xl border border-red-800/40 bg-red-950/20 text-red-400 text-[10px] transition-all cursor-pointer inline-flex items-center gap-1.5 hover:bg-red-900/60 shadow">
          ...
          <div *ngIf="usuariosFiltrados().length === 0" class="py-12 text-center text-gray-500 flex flex-col items-center justify-center gap-2">
            ...
            <span class="text-xs text-gray-400 font-sans">No se encontraron usuarios que coincidan con la búsqueda.</span>
          </div>
        </div>
      </div>
